...
<!-- <object width="500" height="600" id="sample" data="uploads/b2a02.pdf"></object>
 --><div class="container text-center my-5">
    <h5 class="text-uppercase"><b>SELECT YOUR SUBJECT BASED PAPER</b></h5>
...
<div class="col-md-12 offset-md-3">
   <button type="button"   name="but"  onclick="showajaxmodal('tokens')"><img src="img/pdf.png" height="40" width="auto" />
	...
	<div class="my-5">
	<h5 class="text-uppercase" ><b>SELECT YOUR Interdisciplinary BASED PAPER</b></h5>
   <select class="browser-default custom-select col-md-5" name="title2" id="tokens1">
  				<?php 
          $query_inter=mysqli_query($db,"SELECT distinct department FROM interdisciplinary WHERE status='1' AND count!='0'");
		...
		$query_inter1=mysqli_query($db,"SELECT * FROM interdisciplinary WHERE  department ='$dept_inter'  AND status ='1' AND count !='0'");	
		 if (mysqli_num_rows($query_inter1) > 0 ) {
		 	while ($res_inter=mysqli_fetch_array($query_inter1)) {
		 	
 		?>
 		   <option  value="<?php  echo $res_inter['code']; ?> " name="interdisciplinary">	<?php 
...
<?php 
  ...
  if (isset($_GET['submit'])) {
      ...
      $interdisciplinary=$_GET['title2'];


    header("Location: conform.php?title1=$subject_based&title2=$interdisciplinary&id=$id");
  }
